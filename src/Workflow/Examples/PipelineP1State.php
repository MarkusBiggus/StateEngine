...
class PipelineP1State implements WorkflowStateContract
{
    /**
     * run
    ...
    {
        if (isset($appContext->TestSEQTransitions['P1'][$appContext->TestSEQ])) {
            $transition = $appContext->TestSEQTransitions['P1'][$appContext->TestSEQ];
            $appContext->ResetTransitions($transition);
        }

        $appContext->StatePath .= '->P1';

        if ($Workflow::WorkflowDebug) {
            echo("P1: $appContext->StatePath<br/>");
        }
