...
class PipelineP5State implements WorkflowStateContract
{
    /**
     * run
    ...
    {
        if (isset($appContext->TestSEQTransitions['P5'][$appContext->TestSEQ])) {
            $transition = $appContext->TestSEQTransitions['P5'][$appContext->TestSEQ];
            $appContext->ResetTransitions($transition);
        }

        $appContext->StatePath .= '->P5';

        if ($Workflow::WorkflowDebug) {
            echo("P5: $appContext->StatePath<br/>");
        }
