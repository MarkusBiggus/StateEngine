...
class PipelineP6State implements WorkflowStateContract
{
    /**
     * run
    ...
    {
        if (isset($appContext->TestSEQTransitions['P6'][$appContext->TestSEQ])) {
            $transition = $appContext->TestSEQTransitions['P6'][$appContext->TestSEQ];
            $appContext->ResetTransitions($transition);
        }

        $appContext->StatePath .= '->P6';

        if ($Workflow::WorkflowDebug) {
            echo("P6: $appContext->StatePath<br/>");
        }
